    ...
    <footer className="bg-dark-lighter border-t border-white/[0.08]">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          {/* Locations Column */}
          <div>
            <h4 className="text-xs uppercase tracking-wider text-text-secondary mb-4">Top Locations</h4>
            <ul className="space-y-3">
              {topCities.map((city) => (
                <li key={city.slug}>
                  <Link href={`/local-seo/local-seo-agency-${city.slug}/`} className="text-white hover:text-brand transition-colors text-sm">
                    {city.name}
                  </Link>
                </li>
              ))}
